##########################################
#Author: Ajay
#Description: Various operations involving EMAP DAG tree and associated gene expression data
    ...
    for AS in tissueExpression:
        for gene in tissueExpression[AS]:
            if len(tissueExpression[AS][gene])>1:
                ...
                data = Counter(tissueExpression[AS][gene]) #Function to count frequency of different elements in a list
...
def prune_graph(directed_graph, root_node, min_number):
    ...
    for test in check:
        ...
        #print directed_graph.node[child]['name']
        #print directed_graph.node[child]['expression_number']
...
def collapse_exp_node_new2(directed_graph, root_node, expression):
    ...
    for test in check:
        ...
        if child_as=='head' or child_as=='trunk':   ##### Note !!!! Head mesenchyme and Trunk mesenchyme listed under Head and trunk respectively.
            #directed_graph.node[child]['count_cells']=0
            directed_graph.node[child]['gene_collapsed']=deepcopy(genes_child) #debug !!!!
            continue
        #print 'child', child_as
        #genes_child=deepcopy(expression[child_as])
        #directed_graph.node[child]['count_cells']=0
        #print genes_child
        ...
        if 'gene_collapsed' not in directed_graph.node[child].keys():    #if no instance of collapsed gene annotation
            #print directed_graph.node[child].keys()
        ...
        #print directed_graph.node[child]['gene_collapsed']
    check.reverse()
    """
    # Code for printing out the actual indented file
    count=0
    for test in check:
        parent =test[0]
        child =test[1]
        if count==0:
            first_gen_count=len(directed_graph.successors(parent))
        elif count==first_gen_count:
            break
        print directed_graph.node[child]['name']
        count_desc=0
        for successor in directed_graph.successors(child):
            print "\t"+directed_graph.node[successor]['name']
            count_desc+=1
        count+=1
    """
...
#Print out file containing pairwise lineage relationships
def lineage_file_print2(directed_graph, root, lineageFile):
    lineageOut = open(lineageFile, 'w')
    ...
    for oParent, oChild in edgeList:  #directed_graph.edges_iter():
        #print directed_graph.node[oParent]['name'],directed_graph.node[oChild]['name']
        try:
            parentName = directed_graph.node[oParent]['name'] + '_(' + str(directed_graph.node[oParent]['count_cells'])+')'
            childName = directed_graph.node[oChild]['name'] + '_(' + str(directed_graph.node[oChild]['count_cells'])+')'
            lineageOut.write('%s\t%s\n' % (parentName, childName))
        except KeyError:
            #print directed_graph.node[oParent]['name']
            parentName = directed_graph.node[oParent]['name']

            childName = directed_graph.node[oChild]['name'] #+ '_(' + str(directed_graph.node[oChild]['count_cells'])+')'
            lineageOut.write('%s\t%s\n' % (parentName, childName))
            continue

#Output collapsed expression table file
def output_collapsed_expression_tsv (directed_graph,root,out,expression):
    ...
    for oParent, oChild in edgeList:  #directed_graph.edges_iter():
        ...
        #if parentName in list_as: #indent code below if list_as is expected and also add list_as in the arguments of function.
        list_done.append(parentName)
        print_gene_exp=[]
        #print_gene_exp.append(parentName) # prints AS_name
...
def euc_calc_R (directed_graph,root,expression,single_cell_gene_list):
    temp_file='/home/single_cell_dev/lineage_map/tree_roll/temp_table.csv'
    ...
    if len(directed_graph.successors(root))>1: #if node has descendants
        ...
        temp_out.close()
        cmd='Rscript /home/single_cell_dev/lineage_map/source/tree_matching.R'
        result = subprocess.check_output(cmd, shell=True)
        as_node=re.findall('"([^"]*)"',result)
        as_node=as_node[0]
        gate="on"

        #print directed_graph.node[as_node]['name']
        directed_graph.node[as_node]['count_cells']+=1
        return directed_graph,gate,as_node
...
if __name__ == '__main__':
    ...
    fileName = '/home/single_cell_dev/lineage_map/data/emap/biomark_96_mart_export_03232015.tsv' #Reference gene expression table
    #ts='TS'+str(x)
    ts = sys.argv[1] # user input Theiler Stage
    tissueExpression = processGeneTS13(fileName, ts)

    fileName = '/home/single_cell_dev/lineage_map/greedy_runs/lineage_map_collapse/data_files/EMAP_combined.obo' #DAG Tree

    directed_graph = buildGraph(fileName, tissueExpression, ts)
    ...
    lineageFile = '/home/single_cell_dev/lineage_map/tree_roll/' + ts + '_lineage_prune_1.txt'
    new_directed_graph = prune_graph(directed_graph, root_embryo, 1)
    #print ts, len(new_directed_graph.nodes())

    #new_directed_graph2= collapse_exp_node_new(new_directed_graph, root_embryo, tissueExpression)
    new_directed_graph2=collapse_exp_node_new2(new_directed_graph, root_embryo, tissueExpression)

    exp_col_file = '/home/single_cell_dev/lineage_map/tree_roll/' + ts + '_exp_col_prune_1_as_id.tsv'
    output_collapsed_expression_tsv(new_directed_graph2,root_embryo,exp_col_file,tissueExpression)

    cell_data='/home/single_cell_dev/lineage_map/output/all_plates_c5to12_raw_ct_bool.csv'
    os.system('Rscript /home/single_cell_dev/lineage_map/tree_roll/create_Robject.R') # create a background model from cell_data
    
    tree_roll_exp_annot_out='/home/single_cell_dev/lineage_map/tree_roll/treeroll_assignments.tsv'
    cell_graph=tree_roll_master(cell_data,new_directed_graph2,root_embryo,tissueExpression,tree_roll_exp_annot_out)

    cellNumColor, legend2print = assignColorKeys(cell_graph, 30.0)

    fileName = '/home/single_cell_dev/lineage_map/tree_roll/graph_out4.txt'

    output_graph(cell_graph, root_embryo, fileName, cellNumColor, legend2print)

    #print 'finished'
    os.system('cd /home/single_cell_dev/lineage_map/tree_roll; dot -Tpng graph_out4.txt > graph_output4.png')
    lineageFile = '/home/single_cell_dev/lineage_map/tree_roll/lineage_pair_test4.txt'
    lineage_file_print(cell_graph, root_embryo, lineageFile)

    os.system('python tab_print.py  /home/single_cell_dev/lineage_map/tree_roll/lineage_pair_test4.txt > /home/single_cell_dev/lineage_map/tree_roll/lineage_pair_test_out4.txt')
    sys.exit(0)
    exp_col_file = '/home/single_cell_dev/lineage_map/data/exp_num_collapse/' + ts + '_exp_col_prune_1.tsv'
    list_as=['branchial arch','head mesenchyme','head','future brain neural fold','future midbrain','future prosencephalon','future rhombencephalon','future spinal cord','neural fold','cardiovascular system','gut','sensory organ','notochord','paraxial mesenchyme','primitive streak','surface ectoderm','trunk mesenchyme','lateral plate mesenchyme']
    output_collapsed_expression_tsv(new_directed_graph2,root_embryo,list_as,exp_col_file,tissueExpression)
    #output_graph(new_directed_graph2, root_embryo, lineageFile)
